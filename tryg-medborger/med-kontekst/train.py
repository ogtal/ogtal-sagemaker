...
from data_prep import get_data_loader,process_group_model
...
def train(args):
    ...
    # group model
    group_model_df = pd.read_csv(args.group_model)

    # tokenizer,dataloader and model
    tokenizer = AutoTokenizer.from_pretrained(args.model_checkpoint, use_fast=True)
    train_path = os.path.join(args.data_dir,args.train)
    train_loader,train_data = get_data_loader(train_path,group_model_df,tokenizer,args.max_len,args.batch_size)
    ...
    for epoch in range(1, args.epochs + 1):
        ...
        for step, batch in enumerate(train_loader):
            ...
            b_group_feat = batch['group_feat'].to(device)
            
            if args.use_half_precision:
                with torch.cuda.amp.autocast():
                    logits = model(b_input_ids,b_input_mask,b_group_feat)   
    ...
    eval_loader,_ = get_data_loader(eval_path,group_model_df,tokenizer,args.max_len,args.test_batch_size)
...
def test(model, eval_loader,device):
    ...
    print('F1 score:', f'{f1_score(true_labels, predictions,average="macro"):.4f}')
    print('Precision score:', f'{precision_score(true_labels, predictions,average="macro"):.4f}')
    print('Recall score:', f'{recall_score(true_labels, predictions,average="macro"):.4f}')
...
if __name__ == "__main__":
    ...
    parser.add_argument("--group-model", type=str,default='keyword_group_marts_2021.csv')
